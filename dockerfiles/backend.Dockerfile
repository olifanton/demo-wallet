...
RUN apt-get update && apt-get install -y libpng-dev openssl libssl-dev libcurl4-openssl-dev wget git
...
RUN cd /tmp && git clone https://github.com/openswoole/ext-openswoole.git && \
    ...
    phpize  && \
    ./configure --enable-openssl --enable-hook-curl --enable-http2 --enable-sockets && \
    make && make install
